<script lang="ts">
	...
	onMount(() => {
		...
		addMessage({
			id: generateId(),
			text: 'Halo! Saya Venti, asisten AI SAFATANC GROUP. Ada yang bisa saya bantu hari ini?',
			isUser: false,
			timestamp: new Date()
		});
...
<svelte:head>
	<title>Chat dengan Venti AI - SAFATANC GROUP</title>
</svelte:head>
		...
		<div class="mx-auto flex max-w-4xl items-center justify-between">
			<div class="flex items-center space-x-4">
				<a href="/" class="flex items-center gap-3">
					<div class="bg-primary flex h-10 w-10 items-center justify-center rounded-3xl">
						<span class="text-lg font-medium text-black">V</span>
					</div>
					<div>
					...
					<div class="flex {message.isUser ? 'justify-end' : 'justify-start'}">
						<div class="max-w-xs sm:max-w-md lg:max-w-lg xl:max-w-xl">
							<div class="flex items-end gap-2 {message.isUser ? 'flex-row-reverse' : ''}">
								<!-- Avatar -->
								<div
									class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-3xl {message.isUser
										? 'bg-primary'
										: darkMode
											? 'bg-white/10'
											: 'bg-gray-100'}"
